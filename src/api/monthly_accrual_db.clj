...
(defn delete-accrual-records [trade_date]
  (jdbc/with-db-connection [conn db-spec]
                           (jdbc/execute! conn
                                          ["DELETE FROM agent_lending.billing_allocation WHERE trade_date = CAST(? AS DATE)" trade_date])))

(defn accrual-positions-exist [trade-date]
  (jdbc/with-db-connection [conn db-spec]
                           (let [query "SELECT COUNT(*) FROM agent_lending.billing_allocation WHERE trade_date = CAST(? AS DATE)"
